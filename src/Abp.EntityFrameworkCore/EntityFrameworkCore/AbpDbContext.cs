    ...
    public abstract class AbpDbContext : DbContext, ITransientDependency
    {
            ...
            where TEntity : class
        {
            ...
            if (typeof(IMustHaveTenant).IsAssignableFrom(typeof(TEntity)))
            {
                 ...
                 * So, we made a workaround to make it working. It works same as above.
                 */
                Expression<Func<TEntity, bool>> mustHaveTenantFilter = e => ((IMustHaveTenant)e).GetTenantId() == CurrentTenantId || (((IMustHaveTenant)e).GetTenantId() == CurrentTenantId) == IsMustHaveTenantFilterEnabled;
        ...
        protected virtual void CheckAndSetMustHaveTenantIdProperty(object entityAsObj)
        {
            ...
            {
                entity.SetTenantId(currentTenantId.Value);
            }
            else
            {
            ...
            if (entityAsObj is IDeletionAudited)
            {
                ...
                if (entity is IMayHaveTenant || entity is IMustHaveTenant)
                {
                    ...
                    if ((entity is IMayHaveTenant && entity.As<IMayHaveTenant>().TenantId == AbpSession.TenantId) ||
                        (entity is IMustHaveTenant && entity.As<IMustHaveTenant>().GetTenantId() == AbpSession.TenantId))
                    {
